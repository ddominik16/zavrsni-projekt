...
![Pregled](docs/pregled.png)

## Kako pokrenuti


```
git clone https://github.com/ddominik16/zavrsni-projekt.git
cd zavrsni-projekt
...
Datoteka mora biti imenovana `neo4j.dump` jer se ucitava u bazu `neo4j`, i
mora stajati bas u `neo4j/backups/` unutar ove mape — to je jedino mjesto
koje kontejner vidi kao `/backups`.

Ucitavanje ide zasebnom slikom `neo4j/neo4j-admin`, koja je u compose
datoteci pod profilom `tools` pa se ne dize sama. Slika mora biti
`-enterprise` izdanje; Community odbija ovakav dump jer je baza u block
formatu.
...
Sustav preporuka — sedam signala, rezultati razdvojeni po jacini dokaza:

![Preporuke](docs/preporuke.png)

Istrazivac mreze — ego-mreza oko jedne osobe, klik na cvor pomice srediste:

![Mreza](docs/mreza.png)

Analitika — cetiri mjere centralnosti nad istim grafom:

![Analitika](docs/analitika.png)
